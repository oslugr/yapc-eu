...
Pedro Castillo is a longtime Perl user and instructor, is the head of
    ...
    conferences, including CIG and CECI (big Spanish computer science
    event). 
...
Granada is a city accustomed to a large touristic inflow, so its offers a huge number of acommodation options for all budgets. In addition, due to the number of students living in the city (more than 60000 during the year), there are a big amount of visitors in these ages, so there are several economical lodgings.
